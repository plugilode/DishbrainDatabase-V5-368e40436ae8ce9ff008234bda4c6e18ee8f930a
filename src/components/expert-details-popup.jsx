...
const ExpertDetailsPopup = ({ expert, onClose, onUpdate }) => {
  ...
  // Erweitere die render-Funktionen um die Quellenangaben anzuzeigen
  const renderInfoField = (label, name, value, sourceInfo) => (
    <div className="mb-4">
      <label className="block text-sm font-medium text-gray-400 mb-1">{label}</label>
      <InfoSource source={sourceInfo || {
        type: expert[`${name}_source_type`] || 'human',
        url: expert[`${name}_source_url`] || '#',
        date: expert[`${name}_source_date`] || new Date().toISOString(),
        confidence: expert[`${name}_confidence`] || null,
        name: expert[`${name}_source_name`] || 'Expertendatenbank'
      }}>
        <p className="text-gray-200">{value || <span className="text-gray-500">Nicht angegeben</span>}</p>
      </InfoSource>
    </div>
  );

  // Rendere die Fachgebiete des Experten mit Quellenangaben
  const renderExpertise = (expertise) => (
    <div className="space-y-2">
      {expertise?.map((item, index) => (
        <InfoSource key={index} source={{
          type: 'ai',
          url: `https://scholar.google.com/scholar?q=${encodeURIComponent(item)}+${encodeURIComponent(expert.name)}`,
          date: expert.expertise_source_date || new Date().toISOString(),
          confidence: 92,
          name: 'Dishbrain AI Analyse'
        }}>
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 bg-blue-900/30 text-blue-400 rounded-full text-sm border border-blue-800/40">
              {item}
            </span>
          </div>
        </InfoSource>
